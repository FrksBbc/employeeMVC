...
class EmployeeDao
{
    ...
    public function getActiveWorker()
    {
        $dbObj = new Db();
        $conn = $dbObj->getConnection();
        $statement = $conn->prepare("SELECT e.id, e.first_name, e.last_name, e.salary, e.job_start, e.status, ec.name FROM employee as e INNER JOIN employee_category as ec ON e.category_id = ec.id WHERE e.status = 1;");
        $statement->setFetchMode(\PDO::FETCH_OBJ);
        $statement->execute();
        return $statement->fetchAll();
    }
    ...
    public function getAvgSalaryOfDesigners()
    {
        $dbObj = new Db();
        $conn = $dbObj->getConnection();
        $statement = $conn->prepare("SELECT avg(e.salary) as 'avg' FROM employee as e INNER JOIN employee_category as ec ON e.category_id = ec.id WHERE ec.name LIKE 'grafikus';");
        $statement->setFetchMode(\PDO::FETCH_OBJ);
        $statement->execute();
        return $statement->fetch();
    }
    ...
    public function filterEmployees()
    {
        $dbObj = new Db();
        $conn = $dbObj->getConnection();
        $sql = "SELECT e.id, e.first_name, e.last_name, e.salary, e.job_start, e.status, ec.name FROM employee as e INNER JOIN employee_category as ec ON e.category_id = ec.id";

        //Ha a szűrés űrlapon kiválasztottunk egy beosztás, akkor feltétel
        if (isset($_POST['employee']['employeeCategory'])) {
            $employeeCategory = $_POST['employee']['employeeCategory'];
            $sql .= " WHERE e.category_id = $employeeCategory";
        }

        if (isset($_POST['employee']['status'])) {
            $sql .= " AND e.status = 1";
        } else {
            $sql .= " AND e.status = 0";
        }

        if (isset($_POST['employee']['salaryMin']) && $_POST['employee']['salaryMin']!=null) {
            $salaryMin = $_POST['employee']['salaryMin'];
            $sql .= " AND e.salary >=$salaryMin";
        }

        if (isset($_POST['employee']['salaryMax']) && $_POST['employee']['salaryMax']!=null) {
            $salaryMax = $_POST['employee']['salaryMax'];
            $sql .= " AND e.salary <=$salaryMax";
        }
